...
> ⚠️ **别再等官方发这个修复了**：上游作者已在社群明确表示「**原 agent 仓库大概率不会再更新**」（主分支后续仅用于更新镜像，后续可能另写新版本）。也就是说，这个流量修复在官方仓库**只停在源码层、不会再有发布版** —— 想用上只能自己编译一次。
...
1. 上游已基本冻结（作者称大概率不再更新），所以**几乎不用追更**；日后若上游恢复更新，做一次 Sync fork → 重打 tag 即可（见第 5 节）
...
- 上游作者在社群公开建议「**各站长维护自己的仓库**」，主分支后续仅更新镜像 —— 本仓库即按此方向自维护。
