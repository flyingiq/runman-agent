...
- 🚑 **[QUICKSTART.md](QUICKSTART.md)** — 5 分钟根治自更新死循环（自动备份 + SHA256 强校验 + 三条验收断言）
...
1. **永不自更** —— 源码级停更（内部版本恒为 `dev`，updater 直接 return）：服务不会被官方新版本半夜替换；版本恒定、行为可复现，排障少一半变量。
2. **带上官方还没合的修复** —— 官方 `main` 至今不含流量修复（`a90e183`），且**上游作者已明确表示原 agent 仓库大概率不再更新**（主分支后续仅更新镜像）→ 这个修复官方不会再有发布版，**只能自编译拿到**。不合的后果是实测的：流量虚高最高 **182 倍**、以 **+4390 MB/分钟** 疯涨，平台按超额策略把客户实例**误判停机**（客户流失 + 退款）。
3. **控制权在自己手里** —— 源码在手，出问题自己改、推个 tag 由 GitHub Actions 免费出包（约 2 分钟），线上跑哪个 commit 可追溯，不需买 CI、不需本地 Go 环境。

> 代价也直说：上游更新后要**手动同步一次**（Sync fork → 重打 tag → 重新出包）。完整说明见 **[SELFBUILD.md](SELFBUILD.md)** 第 0 节。
